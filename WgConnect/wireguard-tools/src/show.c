...
{
	jni_out_printf("Usage: %s { <interface> | all | interfaces } [public-key | private-key | listen-port | fwmark | peers | preshared-keys | endpoints | allowed-ips | latest-handshakes | transfer | persistent-keepalive | dump]\n", COMMAND_NAME);
}
...
static void dump_print(struct wgdevice *device, bool with_interface)
{
	...
	if (with_interface)
		jni_out_printf("%s\t", device->name);
	jni_out_printf("%s\t", maybe_key(device->private_key, device->flags & WGDEVICE_HAS_PRIVATE_KEY));
	jni_out_printf("%s\t", maybe_key(device->public_key, device->flags & WGDEVICE_HAS_PUBLIC_KEY));
	jni_out_printf("%u\t", device->listen_port);
	if (device->fwmark)
		jni_out_printf("0x%x\n", device->fwmark);
	else
		jni_out_printf("off\n");
	for_each_wgpeer(device, peer) {
		if (with_interface)
			jni_out_printf("%s\t", device->name);
		jni_out_printf("%s\t", key(peer->public_key));
		jni_out_printf("%s\t", maybe_key(peer->preshared_key, peer->flags & WGPEER_HAS_PRESHARED_KEY));
		if (peer->endpoint.addr.sa_family == AF_INET || peer->endpoint.addr.sa_family == AF_INET6)
			jni_out_printf("%s\t", endpoint(&peer->endpoint.addr));
		else
			jni_out_printf("(none)\t");
		if (peer->first_allowedip) {
			for_each_wgallowedip(peer, allowedip)
				jni_out_printf("%s/%u%c", ip(allowedip), allowedip->cidr, allowedip->next_allowedip ? ',' : '\t');
		} else
			jni_out_printf("(none)\t");
		jni_out_printf("%llu\t", (unsigned long long)peer->last_handshake_time.tv_sec);
		jni_out_printf("%" PRIu64 "\t%" PRIu64 "\t", (uint64_t)peer->rx_bytes, (uint64_t)peer->tx_bytes);
		if (peer->persistent_keepalive_interval)
...
static bool ugly_print(struct wgdevice *device, const char *param, bool with_interface)
{
	...
	if (!strcmp(param, "public-key")) {
		...
		jni_out_printf("Invalid parameter: `%s'\n", param);
		show_usage();
		return false;
	}
	return true;
...
int show_main(int argc, const char *argv[])
{
	...
	if (argc == 1 || !strcmp(argv[1], "all")) {
		...
		if (!interfaces) {
			jni_perror("Unable to list interfaces", errno);
			return 1;
		}
		ret = !!*interfaces;
		...
		for (size_t len = 0; (len = strlen(interface)); interface += len + 1) {
			...
			if (ipc_get_device(&device, interface) < 0) {
				jni_err_printf("Unable to access interface %s: %s\n", interface, strerror(errno));
				continue;
			}
            if (argc == 3) {
		...
		interface = interfaces;
		for (size_t len = 0; (len = strlen(interface)); interface += len + 1)
			jni_out_printf("%s%c", interface, strlen(interface + len + 1) ? ' ' : '\n');
		free(interfaces);
