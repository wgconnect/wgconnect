...
const Subcommand subcommands[] = {
	...
	{ "pubkey", pubkey_main, "Generates a public key from the given private key and writes it key to stdout" }
};
size_t subcommands_count = sizeof(subcommands) / sizeof(Subcommand);
