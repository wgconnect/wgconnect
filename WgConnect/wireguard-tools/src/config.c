...
static bool __attribute__((unused))parse_keyfile(uint8_t key[static WG_KEY_LEN], const char *path)
{
	FILE *f;
	int c;
...
struct wgdevice *config_read_cmd(const char *argv[], int argc)
{
	...
	while (argc > 0) {
		if (!strcmp(argv[0], "listen-port") && argc >= 2 && !peer) {
			...
			if (!parse_key(device->private_key, argv[1]))
				goto error;
			...
			if (!parse_key(peer->preshared_key, argv[1]))
				goto error;
