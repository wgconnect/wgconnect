...
static bool sync_conf(struct wgdevice *file)
{
	...
	if (ipc_get_device(&runtime, file->name) != 0) {
		jni_perror("Unable to retrieve current interface configuration", errno);
		return false;
	}
	...
	if (!pubkeys) {
		free_wgdevice(runtime);
		jni_perror("Public key allocation", errno);
		return false;
	}
	...
	for (i = 0; i < peer_count; ++i) {
		...
		if (i == peer_count - 1 || !pubkeys[i + 1].from_file || memcmp(pubkeys[i].pubkey, pubkeys[i + 1].pubkey, WG_KEY_LEN)) {
			...
			if (!peer) {
				free_wgdevice(runtime);
				free(pubkeys);
				jni_perror("Peer allocation", errno);
				return false;
			}
...
int setconf_main(int argc, const char *argv[])
{
	...
	if (argc != 3) {
		jni_err_printf("Usage: %s %s <interface> <configuration filename>\n", PROG_NAME, argv[0]);
		return 1;
	}

	config_input = fopen(argv[2], "r");
	if (!config_input) {
		jni_perror("fopen", errno);
		return 1;
	}
	...
	while (getline(&config_buffer, &config_buffer_len, config_input) >= 0) {
		if (!config_read_line(&ctx, config_buffer)) {
			jni_err_printf("Configuration parsing error\n");
			goto cleanup;
		}
	}
	device = config_read_finish(&ctx);
	if (!device) {
		jni_err_printf("Invalid configuration\n");
		goto cleanup;
	}
